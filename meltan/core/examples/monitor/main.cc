...
#include "meltan/core/examples/monitor/params.pb.h"
#include "meltan/lib/params/utils.h"
#include "meltan/lib/results/results.h"
#include "meltan/lib/results/results.pb.h"

using ::meltan::core::examples::monitor::Params;
using ::meltan::params::GetParams;
...
int main(int argc, char* argv[]) {
  ...
  // Get parameters.
  Params params;
  if (const auto status = meltan::params::GetParams(&params); !status.ok()) {
    test_run->AddError(
  ...
  test_run->StartAndRegisterInfos(std::vector<meltan::results::DutInfo>{},
                                     params);
