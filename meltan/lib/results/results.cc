...
#include "meltan/core/compat/status_macros.h"
...
ABSL_FLAG(bool, meltan_copy_results_to_stdout, true,
          "Prints human-readable result artifacts to stdout in addition to "
          "default output");

ABSL_FLAG(
    std::string, meltan_results_filepath, "",
...
//
ABSL_FLAG(bool, meltan_strict_reporting, true,
          "Whether to require a global devpath to be reported in"
          "third_party.meltan.results_pb.HardwareInfo");
...
constexpr char kSympProceduralErr[] = "meltan-procedural-error";
...
absl::StatusOr<std::unique_ptr<TestRun>> ResultApi::InitializeTestRun(
  ...
  absl::StatusOr<int> fd = internal::OpenAndGetDescriptor(
      absl::GetFlag(FLAGS_meltan_results_filepath).c_str());
  if (!fd.ok()) {
    return fd.status();
  }
  std::ostream* stream =
      absl::GetFlag(FLAGS_meltan_copy_results_to_stdout) ? &std::cout : nullptr;
...
absl::StatusOr<TestRun> TestRun::Init(std::string name) {
  ...
  if (!called) {
    ...
    std::ostream* stream = absl::GetFlag(FLAGS_meltan_copy_results_to_stdout)
                               ? &std::cout
                               : nullptr;
...
TestRun::TestRun(std::string name, internal::ArtifactWriter writer)
    : writer_(std::move(writer)),
      ...
      result_(meltan::results_pb::TestResult::NOT_APPLICABLE),
      status_(meltan::results_pb::TestStatus::UNKNOWN) {}
...
void TestRun::EmitStartUnlocked(absl::Span<const DutInfo> dutinfos,
  ...
  start_pb.set_version(std::string(meltan::params::GetVersion()));
...
void TestRun::WriteLog(meltan::results_pb::Log::Severity severity,
...
void TestStep::Skip() ABSL_LOCKS_EXCLUDED(mutex_) {
  absl::ReleasableMutexLock l(&mutex_);
  if (status_ == rpb::TestStatus::UNKNOWN) {
    status_ = meltan::results_pb::SKIPPED;
  }
  l.Release();
  End();
}
...
void TestStep::WriteLog(meltan::results_pb::Log::Severity severity,
                        absl::string_view msg) {
  rpb::Log log;
...
void MeasurementSeries::AddElementWithRange(
  ...
  *elem_pb.mutable_dut_timestamp() = ::meltan::results::internal::Now();
