...
namespace meltan::results {

void SetFlags(const bool meltan_copy_results_to_stdout,
              const std::string meltan_results_filepath,
              const std::string machine_under_test,
              const bool meltan_strict_reporting) {
  absl::SetFlag(&FLAGS_meltan_copy_results_to_stdout,
                meltan_copy_results_to_stdout);
  absl::SetFlag(&FLAGS_meltan_results_filepath, meltan_results_filepath);
  absl::SetFlag(&FLAGS_machine_under_test, machine_under_test);
  absl::SetFlag(&FLAGS_meltan_strict_reporting, meltan_strict_reporting);
}
      ...
      .def(
          ...
          [](TestStep& a, const int symptom_type, std::string symptom,
            ...
            return a.AddDiagnosis(
                meltan::results_pb::Diagnosis::Type(symptom_type),
