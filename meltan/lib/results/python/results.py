...
"""Wrapper for the meltan results pybind library.

This wrapper is used to set the proper C++ flags for the meltan results library.
...
flags.DEFINE_string(
    ...
    allow_hide_cpp=True)
flags.DEFINE_boolean(
    'meltan_strict_reporting',
    True,
    'Whether to require a global devpath to be reported in third_party.meltan.results_pb.HardwareInfo',
    allow_hide_cpp=True)
...
def _SetPybindFlags():
  SetResultsLibFlags(
      flags.FLAGS.meltan_copy_results_to_stdout,
      flags.FLAGS.meltan_results_filepath, flags.FLAGS.machine_under_test,
      flags.FLAGS.meltan_strict_reporting)
