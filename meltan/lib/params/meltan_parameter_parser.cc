...
#include "meltan/lib/params/meltan_params.pb.h"
...
absl::Status AssignValue(google::protobuf::Message* message,
  ...
  if ((!index.has_value() && MaybeArrayAssignment(field, text)) ||
    ...
    return meltan::params::MergeFromJsonStream(&msg_stream, message);
  }
...
// If the file specifies the (meltan.options).params_message file option, then
...
absl::StatusOr<const google::protobuf::Descriptor*> GetParamsDescriptor(
  ...
  if (!options.HasExtension(meltan::options)) {
  ...
  const meltan::FileOptions& meltan_options =
      options.GetExtension(meltan::options);
  param_type = pool->FindMessageTypeByName(meltan_options.params_message());
  if (param_type == nullptr) {
    return absl::InvalidArgumentError(
        absl::StrCat("Meltan file option (meltan.options).params_message=\"",
                     meltan_options.params_message(),
...
absl::Cord FormatOverrideFlag(
  ...
  if (field->is_repeated() && !array_entry) {
    ...
    cursor = PrettyAppend(&help, cursor, type_name, true);
  }
  help.Append(";");
...
MeltanParameterParser::PrepareExec(Arguments args,
  ...
  if (pos_args.size() < kOptionalJSONDefaults ||
    ...
    return absl::InvalidArgumentError(absl::StrCat(
        ...
        !pos_args.empty() ? pos_args[kLauncherName] : "meltan_launcher",
  ...
  if (absl::Status from_file =
          meltan::params::MergeFromJsonStream(json_stream, params.get());
      !from_file.ok()) {
    return from_file;
  }
