...
class OutputReceiver {
 public:
  ...
  // at a time.
  const ocpdiag_results_pb::TestRunModel &test_run();

  // Returns an iterable container of the raw output artifacts. It can be
  // iterated over as many times as you like.
  const RecordIoContainer<ocpdiag::results_pb::OutputArtifact>
      &raw() {
    return container_;
  }

 private:
  RecordIoContainer<ocpdiag::results_pb::OutputArtifact> container_;
  std::unique_ptr<internal::ArtifactWriter> artifact_writer_;
  std::optional<ocpdiag_results_pb::TestRunModel> model_;
};

// Adds a single output artifact to the object.
absl::Status AddOutputArtifact(
    ocpdiag_results_pb::TestRunModel &model,
