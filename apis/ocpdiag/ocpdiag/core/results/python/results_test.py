...
class TestRunTest(ResultsTestBase):
  ...
  def testTestRunAddError(self):
    ...
    self.assertEqual(self.output().errors[0].symptom, "symptom")

  def testTestRunAddTag(self):
    with self.test_run as test_run:
      test_run.AddTag("T")

    self.assertEqual(len(self.output().tags), 1)
    self.assertEqual(self.output().tags[0].tag, "T")
  ...
  def testTestRunLogs(self):
    ...
    self.assertEqual(self.output().logs.debug[0].text, "A")
    self.assertEqual(self.output().logs.info[0].text, "B")
    self.assertEqual(self.output().logs.warning[0].text, "C")
    self.assertEqual(self.output().logs.error[0].text, "D")
    self.assertEqual(self.output().logs.fatal[0].text, "E")
  ...
  def testTestRunStartAndRegisterInfos(self):
    ...
    self.assertEqual(
        len(self.output().start.dut_info[0].hardware_components), 1)
    self.assertEqual(len(self.output().start.dut_info[0].software_infos), 1)
...
class TestRunStep(ResultsTestBase):
  ...
  def testTestStepBegin(self):
    ...
    self.assertIn("0", self.output().test_steps.keys())
    self.assertEqual(self.output().test_steps["0"].start.name, "TestStepTest")
  ...
  def testTestStepAddDiagnosis(self):
    ...
    self.assertEqual(self.output().test_steps["0"].diagnoses[0].type,
  ...
  def testTestStepAddError(self):
    ...
    errors = self.output().test_steps["0"].errors
  ...
  def testTestStepAddMeasurement(self):
    ...
    measurements = self.output().test_steps["0"].measurements
  ...
  def testTestStepAddFile(self):
    ...
    files = self.output().test_steps["0"].files
  ...
  def testTestStepAddArtifactExtension(self):
    ...
    extensions = self.output().test_steps["0"].artifact_extensions
  ...
  def testTestStepDebug(self):
    ...
    logs = self.output().test_steps["0"].logs
...
class TestMeasurementSeries(ResultsTestBase):
  ...
  def testMeasurementSeriesBegin(self):
    ...
    measurement_series = self.output().test_steps["0"].measurement_series
