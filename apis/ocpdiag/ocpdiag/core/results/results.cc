...
bool TestStep::ValidateRange(const google::protobuf::Value& value,
                             ...
                             const rpb::MeasurementInfo& info,
                             bool enforce_constraints) {
  if (range.has_minimum() && Compare(value, range.minimum()) < 0) {
    if (enforce_constraints) {
      AddError("error-value-too-small",
               absl::StrFormat("From TestStep '%s' measurement '%s': Value "
                               "'%s' is less than the minimum of '%s'",
                               name_, info.name(), value.DebugString(),
                               range.minimum().DebugString()),
               {});
    }
    return false;
  }
  if (range.has_maximum() && Compare(value, range.maximum()) > 0) {
    if (enforce_constraints) {
      AddError("error-value-too-large",
               absl::StrFormat("From TestStep '%s' measurement '%s': Value "
                               "'%s' is more than the maximum of '%s'",
                               name_, info.name(), value.DebugString(),
                               range.maximum().DebugString()),
               {});
    }
    return false;
  }
...
bool TestStep::AddMeasurement(rpb::MeasurementInfo info,
  ...
  bool valid = true;
  if (element.has_range() && !ValidateRange(element.value(), element.range(),
                                            info, enforce_constraints)) {
    valid = false;
  }
...
bool MeasurementSeries::AddElementWithRange(
  ...
  *out_pb.mutable_test_step_artifact() = std::move(step_pb);
  writer_->Write(out_pb);
  return test_step_->ValidateRange(value, range, info_, enforce_constraints_);
}
